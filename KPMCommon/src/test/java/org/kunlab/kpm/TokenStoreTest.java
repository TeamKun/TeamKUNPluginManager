...
class TokenStoreTest
{
    ...
    private static TokenStore createInstance()
    {
        ...
        return new TokenStore(tokenPath, keyPath, Mockito.mock(ExceptionHandler.class));
    }
    ...
    @Test
    @Order(0)
    void 環境変数から読み込めるか() throws Exception
    {
        TokenStore tokenStore = createInstance();

        SystemLambda.withEnvironmentVariable("TOKEN", "dummy_token_114514")
                .execute(() -> {
                    tokenStore.fromEnv();
    ...
    @Test
    @Order(2)
    void トークンがある場合にisTokenAvailableが動くか() throws Exception
    {
        TokenStore tokenStore = createInstance();
        SystemLambda.withEnvironmentVariable("TOKEN", "dummy_token_114514")
                .execute(() -> {
    ...
    void tokenにディレクトリが入らないかどうか()
    {
        ...
        assertThrows(
                ...
                () -> new TokenStore(tokenPath, keyPath, Mockito.mock(ExceptionHandler.class))
        );
    }
    ...
    @EnabledIf("isTokenSet")
    @SneakyThrows(IOException.class)
    void トークンを保管し読み込みできるか() throws Exception
    {
        TokenStore tokenStore = createInstance();
    ...
    void トークンを移行できるか() throws IOException
    {
        ...
        TokenStore tokenStore = new TokenStore(tokenPath, keyPath, Mockito.mock(ExceptionHandler.class));
